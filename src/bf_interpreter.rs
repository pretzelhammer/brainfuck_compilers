use brainfuck_compilers::{ parse, MEM_LIMIT, EOF, Error, Inst};
        ...
        match instructions[inst_ptr] {
            ...
            Inst::WriteByte => {
                ...
                match result {
                    ...
                    Ok(_) => {
                        // wrote 1 byte to output as expected
            ...
            Inst::LoopStart(start_depth) => {
                if memory[ptr] == 0 {
                    inst_ptr += 1;
                    loop {
                        if let Inst::LoopEnd(end_depth) = instructions[inst_ptr] {
            ...
            Inst::LoopEnd(end_depth) => {
                if memory[ptr] != 0 {
                    inst_ptr -= 1;
                    loop {
                        if let Inst::LoopStart(start_depth) = instructions[inst_ptr] {
                            if start_depth == end_depth {
                                break;
                            }
                        }
...
mod tests {
    ...
    #[test]
    fn run_short_hello_world() {
        let src = include_str!("../input/short_hello_world.b");
        let input = [].bytes();
        let mut output = Vec::new();
