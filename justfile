...
compile-and-run-wasm32-wasi-example name:
    docker run -it -v $(pwd):/root/project -w /root/project/examples/wasm32-wasi brainfuck-compilers-env bash -c "wat2wasm {{name}}.wat -o {{name}}.wasm; PATH=/root/.wasmtime/bin wasmtime {{name}}.wasm; echo -e \"\nExit code: \$?\""
...
compile-and-run-bf-to-wasm32-wasi name:
    cargo run --release --bin bf_to_wasm32_wasi_compiler -- ./input/{{name}}.b ./output/wasm32-wasi/{{name}}.wat
    docker run -it -v $(pwd):/root/project -w /root/project/output/wasm32-wasi brainfuck-compilers-env bash -c "wat2wasm {{name}}.wat -o {{name}}.wasm; PATH=/root/.wasmtime/bin wasmtime {{name}}.wasm"
